...
<div class="container-fluid">
    <div class="card conatiner-card">
        <div class="card-header container-card--header">
            <span class="float-left">Listagem de Cidades</span>
            <button class="btn" onclick="window.location.href = '<?=url("admin/cidades/create")?>'" >Novo cadastro</button>
        </div>
        <div class="card-body">
        <table class="table">
            ...
            <?php foreach($cidades as $key => $cidade):?>
                <tr>
                <th scope="row"><?=$cidade->data->id?></th>
                <td><?=$cidade->data->nome?></td>
                <td>
                    <a href="<?=url("admin/cidades/editar?id={$cidade->data->id}")?>">
